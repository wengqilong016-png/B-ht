-- P2: Zero-revenue settlement guard
-- Prevents admins from confirming settlements where totalRevenue <= 0,
-- which would incorrectly mark all driver transactions for that day as 'paid'
-- without actual cash having been collected.
--
-- This migration is idempotent — it replaces the existing function.
...
BEGIN
    ...
    -- Confirming a settlement with zero total revenue would incorrectly mark
    -- all collection transactions for that driver+date as paymentStatus='paid'
    -- and update the driver's dailyFloatingCoins to zero — destroying the
    -- accounting trail for any real collections that arrived later.
    IF p_status = 'confirmed' THEN
        IF COALESCE(v_settlement."totalRevenue", 0) <= 0 THEN
            RAISE EXCEPTION
