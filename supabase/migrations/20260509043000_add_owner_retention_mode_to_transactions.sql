-- Track whether each owner-retention amount was retained or paid on site.
-- Before this, transactions.ownerRetention stored the amount but not the mode,
-- so direct-pay and retained dividends could not be audited per transaction.

ALTER TABLE public.transactions
  ADD COLUMN IF NOT EXISTS "isOwnerRetaining" BOOLEAN;

COMMENT ON COLUMN public.transactions."isOwnerRetaining" IS
  'TRUE when ownerRetention was retained into locations.dividendBalance; FALSE when paid on site; NULL for legacy/unknown rows.';
