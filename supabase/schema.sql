...
-- ── storage bucket / evidence uploads ───────────────────────────────────────
...
BEGIN
    ...
    INSERT INTO public.transactions (
        ...
        v_revenue, v_commission, v_final_retention, COALESCE(p_is_owner_retaining, FALSE),
    ...
    RETURN json_build_object(
        ...
        'isOwnerRetaining',     COALESCE(p_is_owner_retaining, FALSE),
        'debtDeduction',        0,
        'startupDebtDeduction', v_startup_debt_deduction,
...
DECLARE
    ...
    v_message TEXT;
BEGIN
    IF NEW.event_name NOT IN ('submit_success', 'submit_offline_queued', 'submit_failed', 'submit_zero_revenue') THEN
        RETURN NEW;
    END IF;
    ...
    v_score_line := COALESCE(NULLIF(v_payload->>'previousScore', ''), '—') || ' → ' || COALESCE(NULLIF(v_payload->>'currentScore', ''), '—');
    ...
    CASE NEW.event_name
        WHEN 'submit_offline_queued' THEN
            ...
            v_message := v_location_name || '｜' || v_revenue_line || '｜分数 ' || v_score_line || '｜联网同步后管理端可见｜交易号 ' || v_tx_id;
        WHEN 'submit_failed' THEN
            v_type := 'driver_collection_failed';
            v_title := '收款失败：' || v_driver_name;
            v_message := v_location_name || '｜分数 ' || v_score_line || '｜原因：' || v_reason || '｜交易号 ' || v_tx_id;
        WHEN 'submit_zero_revenue' THEN
            v_type := 'driver_collection_zero_revenue';
            v_title := '零营业额异常：' || v_driver_name;
            v_message := v_location_name || '｜' || v_revenue_line || '｜分数 ' || v_score_line || '｜云端已记录但营业额为 0，请核查上次分数｜交易号 ' || v_tx_id;
        ELSE
            IF EXISTS (SELECT 1 FROM public.transactions WHERE id = v_tx_id) THEN
                ...
                v_message := v_location_name || '｜' || v_revenue_line || '｜分数 ' || v_score_line || '｜管理端已可见｜交易号 ' || v_tx_id;
            ELSE
                v_type := 'driver_collection_failed';
                v_title := '成功事件无交易：' || v_driver_name;
                v_message := v_location_name || '｜分数 ' || v_score_line || '｜原本记录为成功，但交易表没有对应交易，请人工核查｜交易号 ' || v_tx_id;
            END IF;
    END CASE;

    INSERT INTO public.notifications (type, title, message, "timestamp", "isRead", "driverId", "relatedTransactionId", "driverFlowEventId")
    VALUES (v_type, v_title, v_message, COALESCE(NEW.created_at, NOW()), FALSE, NEW.driver_id, v_tx_id, NEW.id)
    ON CONFLICT DO NOTHING;
...
FOR EACH ROW
WHEN (NEW.event_name IN ('submit_success', 'submit_offline_queued', 'submit_failed', 'submit_zero_revenue'))
